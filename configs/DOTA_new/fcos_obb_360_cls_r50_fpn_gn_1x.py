...
model = dict(
    ...
    neck=dict(
        ...
        num_outs=5),
    rbbox_head=dict(
        type='FCOSRHead_360_CLS',
        num_classes=16,         # include bg
        num_angle_classes=361,  # include bg
        in_channels=256,
        stacked_convs=4,
        feat_channels=256,
        ...
        # loss_bbox=dict(type='SmoothL1Loss', beta=0.11, loss_weight=0.01),
        # loss_bbox=dict(type='MSELoss', loss_weight=1.0),
        loss_bbox=dict(type='RotatedIoULoss_360',
                       loss_weight=1.0,
                       angle_loss_type='none',  # here, no angle reg loss
...
log_level = 'INFO'
work_dir = './work_dirs/fcos_obb_360_cls_r50_fpn_gn_1x'
load_from = None
resume_from = None
workflow = [('train', 1)]
