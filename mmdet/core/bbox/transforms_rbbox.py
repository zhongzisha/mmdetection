...
def gt_mask_bp_obbs_360(gt_masks, with_module=True):
    ...
    # trans gt_masks to gt_obbs
    # gt_polys = mask2poly(gt_masks)
    gt_bp_polys = get_best_begin_point_360(gt_masks)
...
def get_best_begin_point_for_quad_boxes(quad_boxes):
    new_quad_boxes = []
    for quad_box in quad_boxes:
        ...
        if force_flag != 0:
            pass
            # print("choose one direction!")
        new_quad_boxes.append(np.array(combinate[force_flag]).reshape((1, 8)))
    return np.array(new_quad_boxes).reshape((-1, 8))


def get_best_begin_point_360(quad_boxes):
    new_quad_boxes = []
    for quad_box in quad_boxes:
        x1 = quad_box[0]
        y1 = quad_box[1]
