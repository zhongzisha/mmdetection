...
from mmdet.core.bbox.transforms_rbbox import RotBox2Polys_360
...
def rbbox_overlaps_cy_warp_360(rbboxes, query_boxes):
    # TODO: first calculate the hbb overlaps, for overlaps > 0, calculate the obb overlaps
    # import pdb
    # pdb.set_trace()
    box_device = query_boxes.device
    ...
    # polys_np = mask2poly(gt_masks)
    # polys_np = np.array(Tuplelist2Polylist(polys_np)).astype(np.float)
    rbboxes_np = rbboxes.cpu().numpy().astype(np.float)
    polys_np = RotBox2Polys_360(rbboxes_np).astype(np.float)
    query_polys_np = RotBox2Polys_360(query_boxes_np)
