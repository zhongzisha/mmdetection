...
class SingleStageDetectorRbbox_360(BaseDetectorNew):
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 pretrained=None):
        super(SingleStageDetectorRbbox_360, self).__init__()
    ...
    def init_weights(self, pretrained=None):
        super(SingleStageDetectorRbbox_360, self).init_weights(pretrained)
    ...
    def forward_train(self,
        ...
        if self.with_bbox:
            ...
            for key in bbox_losses:
                for x in bbox_losses[key]:
                    if torch.isnan(x):
                        import pdb
                        pdb.set_trace()
        if self.with_rbbox:
            ...
            for key in rbbox_losses:
                for x in rbbox_losses[key]:
                    if torch.isnan(x):
                        import pdb
                        pdb.set_trace()
