    ...
    def inference_single(self, imagname, slide_size, chip_size):
        ...
        for i in tqdm(range(int(width / slide_w + 1))):
            for j in range(int(height / slide_h) + 1):
                ...
                chip_detections = inference_detector(self.model, subimg)[1]

                # print('result0: ', chip_detections[0])
                # print('result1: ', chip_detections[1])
...
if __name__ == '__main__':
    ...
    input_image_filename = sys.argv[3]
    output_image_filename = sys.argv[4]
    roitransformer = DetectorModel(config_filename, model_filename)

    roitransformer.inference_single_vis(input_image_filename, output_image_filename,
                                        (512, 512), (1024, 1024))
