...
def main():
    ...
    seen = 0
    nc = len(names)
    confusion_matrix = ConfusionMatrix1(nc=nc)
    ...
    for ti in range(len(tiffiles)):
        ...
        save_predictions_to_envi_xml(preds=all_preds.cpu(),
                                     save_xml_filename=os.path.join(str(save_dir), file_prefix + '.xml'),
        ...
        save_predictions_to_envi_xml(preds=all_preds_before.cpu(),
                                     save_xml_filename=os.path.join(str(save_dir), file_prefix + '_before.xml'),
                                     gdal_proj_info=projection_esri,
                                     gdal_trans_info=geotransform,
                                     names=names,
                                     colors=colors,
    ...
    if nl:  # if have gt_boxes
        ...
        if len(stats) and stats[0].any():
            p, r, ap, f1, ap_class = ap_per_class1(*stats, plot=is_plot, save_dir=save_dir, names=names)
